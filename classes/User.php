...
class User extends Database
{
    public function store($request)
    {
        ...
        // execute sql command
        if ($this->conn->query($sql)) {   // success
    ...
    public function login($request)
    {
        ...
        // execute the sql
        if ($result = $this->conn->query($sql)) { // success
            // check if there is a match row
            if ($result->num_rows == 1) { //success
                $user = $result->fetch_assoc();
                // transform into associative array
                    // $password = entered | $user['password] = database
                if (password_verify($password, $user['password'])) { // if match
    ...
    public function update($request,$files)
    {
        ...
        {   // success
            ...
            // check if the user is updating photo
            if($photo){
                // yes
                $sql2 = "UPDATE `users` SET `photo` = '$photo' WHERE `id` = '$id'";
                // setting the destination of the photo that will be uploaded
